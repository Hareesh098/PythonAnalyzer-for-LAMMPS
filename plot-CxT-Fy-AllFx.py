...
for i in range(0,FyNLoop):
 for j in range(0,FxNLoop, 1): #FxNLoop
  ...
  Cx = Cx - Cx[0]
  if(colorbarFlag == 0):
   ax.plot(t,Cx,linewidth="0.8",linestyle="-",label='$f_{x}$ = '+str(Fx[j]))
  elif(colorbarFlag == 1):
   color = cmap(norm(float(Fx[j])))    
   ax.plot(t,Cx,linewidth="0.5",color=color,linestyle="-",label='$f_{x}$ = '+str(Fx[j]))
   #ax.plot(t*Fy,Vx/Fy,linewidth="0.5",color=color,linestyle="-",label='$f_{x}$ = '+str(Fx[j]))

ax.set_xlabel('$t$',fontsize=15)
ax.set_ylabel('$C_{x}(t)-C_{x}(0)$',fontsize=15)
#ax.set_xlim(8e3,1e5)
...
tight_layout()
FigName = f"../plots/Cx-nAtom"+str(nAtom)+"-Z"+str(Z)+"-Fy"+str(Fy[i])+"-AllFx"
