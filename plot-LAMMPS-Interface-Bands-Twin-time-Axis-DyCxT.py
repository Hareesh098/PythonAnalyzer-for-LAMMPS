...
ax1.set_ylabel('$y_{i}^{\prime}(t) - y_{i}^{\prime}(0)$', fontsize=15, color='black')
ax1.set_title('$y_{i}^{\prime}(t) = y_{i}(t) -C_{y}(t)$ of all discs', fontsize=15)
...
# Plotting Cx vs T in the second subplot
VxLabelColor = 'magenta'
ax2 = ax1.twinx()  # Create a twin y-axis for the same subplot
ax2.plot(T_subset * Fy, Cx_subset-Cx_subset[0], linewidth=2.0, linestyle='-', color=VxLabelColor, label='$C_{x}$')
ax2.set_ylabel('$C_{x}(t)-C_{x}(0)$', fontsize=15, color=VxLabelColor)
...
tight_layout()
FigName = f"../plots/DyCxT-Bands-all-grains-BlockInterface-Fy0.0001-Fx0.00006-TwinXaxis-allDisc"
