...
for i in range(nAtomID,nAtomNLoop):
 for j in range(0,FyNLoop):
  ...
  for k in range(0,FxNLoop): #FxNLoop
   ...
   if(colorbarFlag == 0):
    ax.plot(Cx,Cy,linewidth="0.8",linestyle="-",label='$f_{x}$ = '+str(Fx))
   elif(colorbarFlag == 1):
    color = cmap(norm(float(Fx)))    
    ax.plot(Cx,Cy,linewidth="0.5",color=color,linestyle="-",label='$f_{x}$ = '+str(Fx))

ax.set_xlabel('$C_{x}(t) - C_{x}(0)$',fontsize=15)
ax.set_ylabel('$C_{y}(t) - C_{y}(0)$',fontsize=15)
...
tight_layout()
FigName = f"../plots/CxCy-nAtom"+str(nAtom[nAtomID])+"-Z"+str(Z[nAtomID])+"-Fy"+str(Fy)+"-AllFx"
